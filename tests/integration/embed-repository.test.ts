...
describe("EmbedRepository (integração)", () => {
  ...
  it("countActiveByProvider agrupa embeds ativos por provider (painel /admin, PROJECT.md §82)", async () => {
    fake.seed("channel_embeds", [
      { id: 1, channel_id: "a", provider: "ProviderX", active: true },
      { id: 2, channel_id: "b", provider: "ProviderX", active: true },
      { id: 3, channel_id: "c", provider: "ProviderY", active: true },
      { id: 4, channel_id: "d", provider: "ProviderY", active: false },
    ]);
    const repo = createEmbedRepository(asClient(fake));

    const counts = await repo.countActiveByProvider();

    expect(counts).toEqual({ ProviderX: 2, ProviderY: 1 });
  });
});

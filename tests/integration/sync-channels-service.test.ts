...
// getUpstreamChannels faz fetch de verdade pra API externa — nunca
// usar a API real nos testes (PROJECT.md §68). Mockado no nível do
// módulo pra devolver os fixtures sanitizados.
...
describe("syncChannels (integração — fixtures + fake do Supabase, sem rede/banco reais)", () => {
  ...
  it("sincroniza os 6 canais do fixture do zero (todos criados)", async () => {
    ...
    // canal-dois-embeds tem 2 embeds, os outros 5 têm 1 cada = 7
    expect(fake.getTable("channel_embeds")).toHaveLength(7);
    // canal-sem-epg não deveria ter linha em channel_epg
  ...
  it("marca como inativo um canal que já existia mas não veio mais do upstream (PROJECT.md §22)", async () => {
    ...
    // continua existindo — nunca deleta
  ...
  it("uma segunda sincronização concorrente sai como skipped (PROJECT.md §26)", async () => {
    // simula um lock já ocupado, "roubando" a checagem que
    // syncChannels() faz assim que começa.
  ...
  it("libera o lock mesmo quando a sincronização falha", async () => {
    ...
    // se o lock não foi liberado, esta segunda chamada sairia "skipped"

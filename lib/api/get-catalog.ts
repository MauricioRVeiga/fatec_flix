...
/**
 * Busca embeds/EPG de um lote de canais e serializa tudo junto —
 * o mesmo "join + serialize" que toda função deste arquivo precisa.
 * Extraído porque estava reimplementado (com o mesmo `groupBy` +
 * `Map`) em cada uma delas: uma correção no join só precisava ser
 * feita aqui agora, em vez de em 3-4 lugares.
 */
...
/**
 * Todos os canais ativos, já serializados com embeds/EPG — usado
 * pela homepage para montar as prateleiras (PROJECT.md §32) sem
 * repetir consultas por seção. Server-only: chamado direto de Server
 * Components, sem passar pela nossa própria API HTTP.
 */
...
/**
 * Listagem paginada e filtrável (PROJECT.md §28 `/api/channels`,
 * §42 busca) — usada tanto pela API pública quanto pela homepage
 * (modo de busca), pra não duplicar o pipeline "listar + juntar
 * embeds/EPG + serializar" em cada chamador.
 */
...
/** Busca (PROJECT.md §42) usada pela homepage quando `?q=` está presente. */
export async function searchCatalog(q: string, limit = 48): Promise<ListCatalogResult> {
  return listCatalog({ q, page: 1, limit });
}

/**
 * Canal + embeds + EPG para a página individual (PROJECT.md §36).
 * `null` se não existir/inativo.
 *
 * Memoizado com `React.cache` por requisição: tanto `generateMetadata`
 * quanto o componente da página chamam isso para o mesmo `id`, e sem
 * isso seria uma query a mais no Supabase por request à toa.
 */
...
/** Outros canais da mesma categoria (PROJECT.md §36 "Outros canais relacionados"). */
...
/**
 * Resolve um slug de URL (`/categoria/[slug]`) para o nome real da
 * categoria no banco (PROJECT.md §43) — nunca assume que o slug bate
 * com o nome formatado, sempre compara via `slugify()` contra as
 * categorias que existem de verdade.
 */
